...
export default function Update() {
  ...
  const handleSubmit = (val) => {
    ...
    if (val.image && val.image.file) {
      console.log('with image', updateValues);
      // convertFile(val.image.file.originFileObj);
    } else {
      console.log('without image', updateValues);
      // fetch(`${process.env.REACT_APP_ENDPOINT}/inventory/update/${oid}`, {
      //   method: "POST",
      //   headers: {
      //     'Content-Type': 'application/json',
    ...
    .then((res) => {
      if (typeof res.message === 'object') {
        if (res.message.replaceBy) {
          console.log(res.message.replaceBy)
          const months = {
            ...
            <Collapse accordion>
              ...
              <Panel header="Options" key="2">
                ...
                <Input name='treshold'
                  ...
                  <small>Current reminder date {values?.threshold}</small>
              </Panel>
            </Collapse>
